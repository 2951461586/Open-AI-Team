...
async function createExecutionContext({ paths = {}, state = {}, manifest = {}, manifestPath = '', agentPackage = {}, agentPackagePath = '', workerEntryPath = DEFAULT_WORKER_ENTRY, brokerEntryPath = DEFAULT_BROKER_ENTRY } = {}) {
  ...
  const runtimeAdapter = createRemoteBrokerRuntimeAdapter({
    manifestPath: String(manifestPath || path.join(__dirname, 'oss-agent-manifest.json')),
    agentPackagePath: String(agentPackagePath || path.join(__dirname, 'oss-agent-package.json')),
    workerEntryPath,
    brokerEntryPath,
    paths,
    state,
...
export async function createStandaloneProductRuntime({
  ...
  const manifestDir = path.dirname(path.resolve(String(manifestPath || path.join(__dirname, 'oss-agent-manifest.json'))));
  const workerEntrySpec = String(manifest?.runtime?.workerEntry || DEFAULT_WORKER_ENTRY);
  const brokerEntrySpec = String(manifest?.runtime?.brokerEntry || DEFAULT_BROKER_ENTRY);
  const workerEntryPath = path.isAbsolute(workerEntrySpec) ? workerEntrySpec : path.resolve(manifestDir, workerEntrySpec);
  const brokerEntryPath = path.isAbsolute(brokerEntrySpec) ? brokerEntrySpec : path.resolve(manifestDir, brokerEntrySpec);
  ...
  async function createRun(userText = '') {
    ...
    const context = await createExecutionContext({ paths, state, manifest, manifestPath, agentPackage, agentPackagePath, workerEntryPath, brokerEntryPath });
