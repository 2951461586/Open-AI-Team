...
try {
  ...
  const agentPackageSpec = payload.agentPackagePath || new URL('./oss-agent-package.json', import.meta.url);
  const agentPackagePath = String(agentPackageSpec).startsWith('file:') ? fileURLToPath(agentPackageSpec) : String(agentPackageSpec);
