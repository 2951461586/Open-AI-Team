...
import { createLlmExecutionEnvelope } from './execution-envelope.mjs';
  ...
  async execute(request = {}, options = {}) {
    ...
    for (let index = 0; index < attempts.length; index += 1) {
      ...
      try {
        ...
        const envelope = createLlmExecutionEnvelope({
          request,
          response: enriched,
        ...
        enriched.execution = envelope;
        if (this.auditLogger) {
          await this.auditLogger({ ok: !!enriched?.ok, request, response: enriched, selectedModel: selected, routeInfo, startedAt, finishedAt, fallbackUsed: index > 0, envelope });
        }
        lastResult = enriched;
        ...
        const envelope = createLlmExecutionEnvelope({
          request,
          response: { ok: false, error: String(error?.message || error || 'model_router_attempt_failed') },
          selectedModel: selected,
          routeInfo,
          fallbackUsed: index > 0,
          startedAt,
        ...
        if (this.auditLogger) {
          await this.auditLogger({ ok: false, request, error: String(error?.message || error || 'model_router_attempt_failed'), selectedModel: selected, routeInfo, startedAt, finishedAt, fallbackUsed: index > 0, envelope });
        }
      }
    }
