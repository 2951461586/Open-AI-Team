...
export function createHostAgnosticBootstrap(config = {}) {
  ...
  const controlPlaneSystemdUnit = String(hostConfig?.local?.controlPlaneSystemdUnit || hostConfig?.serviceEnvUnit || 'liziai-api.service');
  return {
    hostKind: 'host-agnostic',
    createNodeHealth({ teamStore = null } = {}) {
      return createTeamNodeHealth({ teamStore, controlPlaneSystemdUnit });
    },
