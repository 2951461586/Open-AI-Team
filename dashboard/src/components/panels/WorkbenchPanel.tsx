        ...
        {taskId && !loading && !error && (
          <>
            ...
            <SummaryHeroSection
              ...
              latestDecisionText={latestDecision ? decisionTypeLabel(latestDecision) : null}
              ...
              acceptancePulse={acceptancePulse}
              acceptanceTone={task?.deliverableReady ? 'border-[var(--success)]/20 bg-[var(--success-soft)]/55' : issueCount > 0 ? 'border-[var(--warning)]/20 bg-[var(--warning-soft)]/55' : 'border-[var(--border-subtle)] bg-[var(--surface)]/95'}
              coreFacts={coreFacts.slice(0, 3)}
              routeMode={routeMode}
              sessionModeText={sessionModeText}
              sessionPersistent={sessionPersistent}
              actualNodeLabel={(summaryPayload?.actualNode || task?.actualNode) ? nodeLabel(String(summaryPayload?.actualNode || task?.actualNode || '')) : ''}
              sessionHint={sessionHint}
              degradedReasonText={(summaryPayload?.degradedReason || task?.degradedReason) ? degradedReasonLabel(String(summaryPayload?.degradedReason || task?.degradedReason || '')) : ''}
              manualActionCount={manualActions.length}
              busyPeopleCount={busyPeopleCount}
              totalPeopleCount={peopleView.length || 0}
