...
- [x] P4.2 后端 WS 服务端（已加到 src/index.mjs）
- [x] P4.3 物理聊天流式增强（已修改 generateReplyStream）
- [x] P4.4 前端 stream 事件处理（已修复支持非 task 流式）
- [ ] P4.5 端到端测试（需要部署后验证）

## 已完成
1. **src/index.mjs**: 添加 WebSocket 服务器，路径 `/ws/chat`
2. **src/team/team-physical-chat.mjs**: 修复 generateReplyStream 返回完整内容
...
- [ ] 验证 WS 连接和流式响应
- [ ] 检查配置中的公开 API 域名的 443 端口是否正确转发到 orchestrator
