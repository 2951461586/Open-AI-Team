...
export function createLocalToolRuntime({ sandbox, eventBus = null, outputDir = '', memoryProvider = null, commandRuntime = null, bridgeHost = null, lifecycleRuntime = null, productShell = null, capabilityGate = null, initialRuns = [], toolRegistry = [], providers = [], audit = null, roles = {}, schemaPath = '', runtimeId = 'local-tool-runtime' } = {}) {
  ...
  async function callTool(toolName = '', args = {}, context = {}) {
    ...
    if (!tool) {
      ...
      await recordRun(toolName, args, result, false, { startedAt, finishedAt: nowIso(), durationMs: 0, actor, source: 'missing' });
      return { ok: false, tool: toolName, result };
    }
    ...
    if (!roleCheck.ok) {
      ...
      await recordRun(tool.id, args, result, false, { startedAt, finishedAt: nowIso(), durationMs: 0, actor, auditId: auditEntry.auditId, source: tool.source?.name || tool.source?.type || '' });
      return { ok: false, tool: tool.id, result };
    }
    ...
    try {
      ...
      await recordRun(tool.id, args, result, true, { startedAt, finishedAt, durationMs, actor, auditId: auditEntry.auditId, source: tool.source?.name || tool.source?.type || '' });
      return { ok: true, tool: tool.id, result };
    } catch (error) {
      const finishedAt = nowIso();
      const durationMs = Math.max(0, Date.parse(finishedAt) - Date.parse(startedAt));
      const result = {
      ...
      await recordRun(tool.id, args, result, false, { startedAt, finishedAt, durationMs, actor, auditId: auditEntry.auditId, source: tool.source?.name || tool.source?.type || '' });
      return { ok: false, tool: tool.id, result };
    }
  }
